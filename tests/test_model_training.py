...
from config import LR_C_OPTIONS, RF_N_ESTIMATORS_OPTIONS, RF_MAX_DEPTH_OPTIONS, RF_MIN_SAMPLES_SPLIT_OPTIONS, RF_MIN_SAMPLES_LEAF_OPTIONS, XGB_N_ESTIMATORS_OPTIONS, XGB_LEARNING_RATE_OPTIONS, CV_FOLDS
...
def test_train_evaluate_model_lr_n_jobs(mock_mlflow):
    with patch('model_training.DASK_TYPE', 'local'):
        with patch('model_training.RandomizedSearchCV') as mock_randomized_search_cv:
            ...
            assert mock_randomized_search_cv.call_args[1]['n_iter'] == 11
            assert mock_randomized_search_cv.call_args[1]['n_jobs'] == 1
            assert mock_randomized_search_cv.call_args[1]['verbose'] == 1

def test_train_evaluate_model_rf_param_grid(mock_mlflow):
    with patch('model_training.DASK_TYPE', 'local'):
        with patch('model_training.RandomizedSearchCV') as mock_randomized_search_cv:
            # Configure the mock to return a mock estimator
            mock_instance = MagicMock()
            ...
            assert mock_randomized_search_cv.call_args[1]['cv'] == CV_FOLDS
            assert mock_randomized_search_cv.call_args[1]['scoring'] == 'roc_auc'
            assert mock_randomized_search_cv.call_args[1]['n_iter'] == 9
            assert mock_randomized_search_cv.call_args[1]['n_jobs'] == -1
            assert mock_randomized_search_cv.call_args[1]['verbose'] == 1

def test_train_evaluate_model_xgb_param_grid(mock_mlflow):
    with patch('model_training.DASK_TYPE', 'local'):
        with patch('model_training.RandomizedSearchCV') as mock_randomized_search_cv:
            # Configure the mock to return a mock estimator
            mock_instance = MagicMock()
