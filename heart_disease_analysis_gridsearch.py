...
if __name__ == "__main__":
    ...
    # 2. Harmonize and Combine Datasets
        ...
        if os.path.exists(X_cache_path) and os.path.exists(y_cache_path) and os.path.exists(preprocessor_cache_path):
            ...
            X_combined = joblib.load(X_cache_path)
            y_combined = joblib.load(y_cache_path)
            preprocessor_combined = joblib.load(preprocessor_cache_path)
            print("Preprocessed data loaded from cache.")
        else:
            print("Preprocessing data and saving to cache...")
            ...
            joblib.dump(X_combined, X_cache_path)
            joblib.dump(y_combined, y_cache_path)
            joblib.dump(preprocessor_combined, preprocessor_cache_path)
        ...
        if X_combined is not None:
            ...
            lr_model_combined, lr_y_pred_combined, lr_y_proba_combined, lr_metrics_combined = train_evaluate_model(
                X_combined, y_combined, preprocessor_combined, model_type='logistic_regression', param_grid=param_grid_lr, dask_client=dask_client
            )
            ...
            rf_model_combined, rf_y_pred_combined, rf_y_proba_combined, rf_metrics_combined = train_evaluate_model(
                X_combined, y_combined, preprocessor_combined, model_type='random_forest', param_grid=param_grid_rf, dask_client=dask_client
            )
            ...
            xgb_model_combined, xgb_y_pred_combined, xgb_y_proba_combined, xgb_metrics_combined = train_evaluate_model(
                X_combined, y_combined, preprocessor_combined, model_type='xgboost', param_grid=param_grid_xgb, dask_client=dask_client
            )
    ...
    print(f"--- Analysis Complete on Combined Dataset --- Total time: {end_time_total - start_time_total:.2f} seconds.")
