...
import logging
import pandas as pd # Added for pd.Int64Dtype()
import numpy as np # Added for np.float64
...
def load_and_harmonize_uci(file_path, schema_dtypes):
    ...
    df = dd.read_csv(file_path, na_values=['?'], dtype=schema_dtypes)
...
def preprocess_data(df):
    ...
    df = df.rename(columns={'target': HeartDiseaseSchema.TARGET_COLUMN})
